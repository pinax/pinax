...
def restructuredtext(value, **overrides):
    """
    The Django version of this markup filter has an issue when only one title
    or subtitle is supplied in that they are dropped from the markup. This is
    due to the use of "fragment" instead of something like "html_body". We do
    not want to use "html_body" either due to some header/footer stuff we want
    to prevent, but we want to keep the title and subtitle. So we include them
    if present.
    """
